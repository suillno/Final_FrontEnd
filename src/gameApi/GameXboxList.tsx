...
const MainTitle = styled.h2<{ $isSidebarOpen: boolean }>`
  ...
  transition: margin-left 0.3s ease;
  background: linear-gradient(90deg, #107c10, #107c10);
...
const GameXboxList: React.FC = () => {
  ...
  const getPlatformList = (pageCount: number) => {
    ...
    apiGetGamesByPlatform("186,1,14", pageCount)
      .then((res) => {
        if (!res) return;
        // 기존 게임 결과에 새 결과 추가
        const results =
  ...
  return (
    <div className="bg-[#1e1f24] text-white py-6 w-full mt-10">
      {/* 상단 제목 */}
      <MainTitle $isSidebarOpen={isSidebarOpen}>🔵 Xbox</MainTitle>
